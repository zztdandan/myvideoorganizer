#!/usr/bin/env python3
"""
video-actor-organizer: execute_plan.py
执行演员分类计划
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="执行演员分类计划")
...
def execute_operation(operation: dict, dry_run: bool = False) -> dict:
    ...
    if dry_run:
        result["success"] = True
        result["message"] = f"[DRY RUN] Would move: {source.name} -> {destination}"
        return result
    ...
    try:
        ...
        result["success"] = True
        result["message"] = f"Moved: {source.name} -> {operation.get('category', '')}/{operation.get('actor', '')}"
