#!/usr/bin/env python3
"""
video-big-detector: execute_plan.py
执行超宽视频移动计划
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="执行超宽视频移动计划")
...
def execute_operation(operation: dict, dry_run: bool = False) -> dict:
    ...
    if dry_run:
        result["success"] = True
        result["message"] = f"[DRY RUN] Would move: {source.name} ({operation.get('width', 0)}x{operation.get('height', 0)})"
        return result
    ...
    try:
        ...
        result["success"] = True
        result["message"] = f"Moved: {source.name} ({operation.get('width', 0)}x{operation.get('height', 0)})"
