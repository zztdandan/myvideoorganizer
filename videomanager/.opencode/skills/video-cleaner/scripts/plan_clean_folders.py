...
def parse_args():
    ...
    parser.add_argument(
        "--min-size", type=int, default=300, help="最小视频大小(MB) (默认: 300)"
    )
    parser.add_argument(
...
def folder_has_valid_video(
    folder: Path, video_extensions: set, min_size_bytes: int
) -> bool:
    """检查文件夹是否包含有效视频"""
...
def scan_folders(
    root_path: Path, video_extensions: set, min_size_mb: int, delete_dir: str
):
    """扫描目录，找出不含有效视频的文件夹"""
    ...
    try:
        subdirs = [
            d for d in root_path.iterdir() if d.is_dir() and d.name != delete_dir
        ]
    except OSError as e:
...
def main():
    ...
    operations = scan_folders(
        root_path, video_extensions, args.min_size, args.delete_dir
    )
