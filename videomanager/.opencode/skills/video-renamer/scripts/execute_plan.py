...
def parse_args():
    parser = argparse.ArgumentParser(description="执行重命名计划")
    parser.add_argument("--plan", required=True, help="计划 JSON 文件路径")
    parser.add_argument("--dry-run", action="store_true", help="模拟执行，不实际重命名")
...
def execute_operation(operation: dict, dry_run: bool = False) -> dict:
    ...
    if dry_run:
        result["success"] = True
        result["message"] = f"[DRY RUN] Would rename: {source.name} -> {destination.name}"
        return result
    
    try:
        source.rename(destination)
        result["success"] = True
        result["message"] = f"Renamed: {source.name} -> {destination.name}"
