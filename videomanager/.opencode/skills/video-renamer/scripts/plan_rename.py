...
def parse_args():
    ...
    parser.add_argument("--pattern", default="number2", help="序号模式: number|letter|number2")
...
def scan_folders(root_path: Path, pattern: str) -> list:
    """扫描目录，找出需要重命名的文件夹"""
    operations = []
    video_extensions = {".mp4", ".mkv", ".avi", ".wmv", ".mov", ".flv", ".rmvb", ".rm", ".m4v", ".m2ts", ".ts", ".mpg"}
    ...
    try:
        for folder in root_path.iterdir():
            ...
            # 如果文件名已包含 -cd，跳过
            if all("-cd" in f.stem for f in video_files):
                continue
            ...
            for i, video_file in enumerate(video_files, 1):
                new_name = f"{common_prefix}{generate_suffix(i, pattern)}{video_file.suffix}"
...
def main():
    ...
    operations = scan_folders(root_path, args.pattern)
